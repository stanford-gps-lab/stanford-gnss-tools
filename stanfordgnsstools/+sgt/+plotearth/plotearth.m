...
if ~isempty(neomap) % NEO map
    ...
    % add path to this function in no path provided
    % effectively assume that one of the bundled images is being requested
    if isempty(pname)
        currpath = mfilename('fullpath');
        [fpath, ~, ~] = fileparts(currpath);
        mapfname = fullfile(fpath, mapfname);
    end
