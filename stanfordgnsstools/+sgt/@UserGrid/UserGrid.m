classdef UserGrid < matlab.mixin.Copyable
    ...
    %   A UserGrid is a container for a distribution of locations that
    %   users occupy. A UserGrid will also contain all user objects as a
    %   property.
    %
    %   userGrid = sgt.UserGrid(posLLH, varargin) creates user(s) at the
    %   (lat [deg], lon [deg], alt [m]) positions specified in posLLH.
    %   posLLH should be an Nx3 matrix for the creation of N users.
    %   userGrid contains meta data concerning the users that have been
    %   created.
    %
