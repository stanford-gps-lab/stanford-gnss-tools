...
if (nargin > 1)
    ...
    if (res.Globe3D)
        ...
        if (~isempty(res.PolygonFile))
            ...
            fill3(polygonECEF(:,1), polygonECEF(:,2), polygonECEF(:,3), 'y', 'FaceAlpha', 0.5)
        end
        ...
        % Plot user locations on map
        plot(coastData.long, coastData.lat, 'k'); hold on;
        plot(obj.GridPositionLLH(:,2), obj.GridPositionLLH(:,1), 'k.')
        % Plot polygon if requested
        if (~isempty(res.PolygonFile))
            ...
            if (sum(inBound) > 0)
                plot(obj.GridPositionLLH(inBound, 2), obj.GridPositionLLH(inBound, 1), 'r.')
            end
        end
    end
else
