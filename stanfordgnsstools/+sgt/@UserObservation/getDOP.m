...
for i = 1:length(obj)
    ...
    if nargout == 1  % just want dops
        varargout{1} = [varargout{1}; DOPs];
    elseif nargout == 5
        varargout{1} = [varargout{1}; sqrt(sum(DOPs))];         % Compute GDOP.
        varargout{2} = [varargout{2}; sqrt(sum(DOPs(1:3)))];    % Compute PDOP.
        varargout{3} = [varargout{3}; sqrt(DOPs(4))];           % Compute TDOP.
        varargout{4} = [varargout{4}; sqrt(sum(DOPs(1:2)))];    % Compute HDOP.
        varargout{5} = [varargout{5}; sqrt(DOPs(3))];           % Compute VDOP.
    else
