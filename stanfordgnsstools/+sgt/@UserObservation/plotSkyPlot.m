...
for i = 1:length(obj)
    ...
    polarscatter(obj(i).AzimuthAngles, 90 - obj(i).ElevationAngles.*180/pi, sz, prnInView, 'filled');
    
    % Text to be added next to each point
    if length(obj) == 1
        textStr = cellstr(strcat('PRN ', num2str(prnInView)));
        dx = 0*pi/180; dy = 10;
        text(obj(i).AzimuthAngles+dx, 90 - obj(i).ElevationAngles.*180/pi+dy, textStr)
    end   
end
rlim([0 90])
